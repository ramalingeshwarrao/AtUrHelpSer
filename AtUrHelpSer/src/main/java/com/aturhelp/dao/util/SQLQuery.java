...
public class SQLQuery {
	...
	public static final String INSERT_ROUTE = "INSERT INTO milk_route (subject, route_id, provider_id, is_active) VALUES (?, ?, ?, ?)";
	...
	public static final String GET_COUNT_MILK_DATA = "SELECT count(1) from milk_flat_no mfn inner join milk_room mr on mfn.id = mr.room_id inner join milk_appartment ma on ma.id = mfn.app_id inner join milk_route mroute on mroute.id = ma.route_id where mfn.provider_id=? and mfn.is_active=1 and mroute.is_active=true"; 
	...
	public static final String GET_TIMER_DAY_MILK_BY_ROUTE_ID = "SELECT ma.subject as appsubject, ma.name, mfn.room_id, mr.route_id, mp.milkid, mp.cost, mt.quantity, mp.id as pid, mfn.id as mfnid  FROM milk_timer mt INNER JOIN milk_flat_no mfn ON mfn.id = mt.rid INNER JOIN milk_packats mp ON mp.id = mt.mid INNER JOIN milk_appartment ma ON ma.id = mfn.app_id INNER JOIN milk_route mr ON mr.id = ma.route_id INNER JOIN milk_room mroom ON mroom.room_id = mfn.id AND mroom.milk_id = mp.id WHERE mt.milkdate= ? AND ma.route_id = ? AND mt.provider_id = ? AND mr.is_active=true order by ma.priority, ma.name, mfn.room_id";
	
	public static final String GET_DAY_MILK = "SELECT ma.subject as appsubject, ma.name, mfn.id as room_id, mr.route_id, mp.id as milkid, mp.cost, mroom.quantity, mroom.is_alternative, mroom.provider_id FROM milk_appartment ma INNER JOIN milk_flat_no mfn ON ma.id = mfn.app_id AND mfn.is_active = 1 INNER JOIN milk_route mr ON ma.route_id = mr.id INNER JOIN milk_room mroom ON mroom.room_id = mfn.id INNER JOIN milk_packats mp ON mp.id = mroom.milk_id WHERE mfn.id NOT IN (select rid from milk_nomilk where fromdate <= ? AND isUpdated=0  OR fromdate <= ? AND todate >= ? AND provider_id=?) AND ma.provider_id=? AND mr.is_active=true order by ma.name, mfn.room_id";
	public static final String GET_NO_MILK_BY_ROOM_ID_APP_ID = "select mn.id, ma.name, mfn.room_id, mn.fromdate from milk_nomilk mn INNER JOIN milk_flat_no mfn ON mn.rid = mfn.id INNER JOIN milk_appartment ma ON ma.id = mfn.app_id INNER JOIN milk_route mroute ON mroute.id = ma.route_id WHERE isUpdated=? AND ma.id=? AND mfn.id=? AND mfn.provider_id=? AND mroute.is_active=true";
	...
	public static final String GET_ALL_NO_MILK_BY_ROOM_ID_APP_ID = "select ma.name, mfn.room_id, mn.fromdate, mn.todate from milk_nomilk mn INNER JOIN milk_flat_no mfn ON mn.rid = mfn.id INNER JOIN milk_appartment ma ON ma.id = mfn.app_id INNER JOIN milk_route mroute ON mroute.id = ma.route_id WHERE ma.id=? AND mfn.id=? AND mfn.provider_id=? AND mroute.is_active=true";
	public static final String GET_MILK_COST_BY_APP_ID = "select mfn.id, mfn.room_id, sum(cost) as cost, count(1) as quantity from milk_flat_no mfn inner join milk_room mr on mfn.id = mr.room_id inner join milk_packats mp on mp.id = mr.milk_id inner join milk_appartment ma on ma.id = mfn.app_id inner join milk_route mroute on mroute.id = ma.route_id where app_id=? AND mfn.provider_id=? AND mroute.is_active=true group by mfn.room_id order by mfn.room_id";
